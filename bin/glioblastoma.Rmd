...
pbmc <- CreateSeuratObject(counts = pbmc.data, project = projectname, min.cells = min_cells, min.features = min.features)
pbmc
```

```{r}
# Lets examine a few genes in the first thirty cells
pbmc.data[c("GNB1","TP73","PARK7"), 1:30]
```
...
```{r}
pbmc <- subset(pbmc, subset = nFeature_RNA > min_feature_counts & nFeature_RNA < max_feature_counts & percent.mt < max_perc_mitocho)
```
...
pbmc <- FindVariableFeatures(pbmc, selection.method = 'vst', nfeatures = num_features)
...
pbmc <- FindNeighbors(pbmc, dims = 1:dimensionality)
...
pbmc <- RunUMAP(pbmc, dims = 1:dimensionality)
```
...
VlnPlot(pbmc, features = c("PLP1", "MAG"))
# you can plot raw counts as well
VlnPlot(pbmc, features = c("OLR1", "PLAUR"), slot = 'counts', log = TRUE)
FeaturePlot(pbmc, features = c("PLP1", "OLR1", "LPL", "CXCL8", "PLA2G2A", "SERPINE1", "PPP1R1B", "DCX", "CSF1R", "CHI3L1", "CX3CL1", "COL4A1", "CCL5", "CLDN5"))
```
